...
if os.environ.get('USE_SAGEATTN', '0') == '1':
    ...
    _pytorch_sdpa = nn.functional.scaled_dot_product_attention

    def scaled_dot_product_attention(q, k, v, **kwargs):
        # SageAttention only supports fp16/bf16, fallback to SDPA for float32
        if q.dtype in (torch.float16, torch.bfloat16):
            return sageattn(q, k, v)
        else:
            return _pytorch_sdpa(q, k, v, **kwargs)
